...
cherry_pick() {
  ...
  output=''
  debug output test -d "${GITHUB_WORKSPACE}/.git" && git -C "${GITHUB_WORKSPACE}" checkout -b "${branch}" -t "origin/${branch}" || git clone -q --no-tags -b "${branch}" "${repository}" "${GITHUB_WORKSPACE}" || fail "Unable to clone from repository \`${repository}\` a branch named \`${branch}\`, this should not have happened" &&
  (
    cd "${GITHUB_WORKSPACE}"
